...
namespace ConsoleAdoDotNet
{
    public class EmployeeService
    {
        ...
        public async Task ViewAllEmployes()
        {
            Console.WriteLine("\nAll Employees");
        ...
        public async Task UpdateEmployee()
        {
            ...
            using (MySqlConnection connection = new MySqlConnection(_connectionString))
            {
                ...
                try
                {
                    ...
                    var inIntrepresentation = Convert.ToInt32(employeeCount);
                    if (inIntrepresentation == 0)
                    {
                        Console.WriteLine("Employee not found.");
                        return;
                    }
            ...
            using (MySqlConnection connection = new MySqlConnection(_connectionString))
            {
                string query = "UPDATE Employees SET " +
                               ...
                               "Department = IFNULL(@Department, Department), " +
                               "HireDate = IFNULL(@HireDate, HireDate), " +
                               "Salary = IFNULL(@Salary, Salary) " +
                               "WHERE Id = @Id";
